...
trust_tweets$sent_response = SentimentAnalysis::convertToBinaryResponse(trust_tweets$sent_score)

for (i in 1:nrow(trust_tweets)) {
  trust_tweets$senti[i] = analyzeSentiment(
    trust_tweets$word[i],
    language = "english",
...
dict <-
  generateDictionary(trust_tweets$word, trust_tweets$sent_score)
